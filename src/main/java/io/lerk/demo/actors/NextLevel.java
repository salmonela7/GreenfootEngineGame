...
public class NextLevel extends Actor
{
    ...
    public void act()
    {
        ...
        Counter++;
        if(getOneObjectAtOffset(0, 0, Adventurer.class) != null)
        {
            if (getWorld() instanceof KeyScrollWorld)
            {
            ...
            if (getWorld() instanceof Level3 && touched == false)
            {
                touched = true;
                textOne end1 = new textOne();
                getWorld().addObject(end1, 400, -20);
                textTwo end2 = new textTwo();
                getWorld().addObject(end2, 400, 620);
            }
        }
        else return;
    }
